...
这是 EnterpriseMind Agent Runtime 的失败案例（badcase）数据集，用于跑出 **before/after 治理对比指标**。

- **before**：模拟"裸 LLM / 关闭治理"链路的观测结果（无 PromptGuard、无 ACL 下推、无审批门、无 SideEffect Ledger、MemorySaver 级易失状态）。
- **after**：完整 Harness 治理链路下同一输入的观测结果。
...
字段与 `app/schemas/safety.py::SafetyEvalCase` 对齐，额外增加叙事字段：

```jsonc
{
  "id": "bc_inj_001",                  // 用例 ID（SafetyEvalCase.id）
  "category": "prompt_injection",      // SafetyRiskCategory 枚举值（评测引擎分派用）
  "violation_type": "prompt_injection",// 叙事分类学标签（7 类，见下）
  "title": "……",                       // 人类可读标题
  ...
  "narrative": "……",                   // 这条 badcase 的来龙去脉（面试讲述素材）
...
1. 线上/演示中出现失败 → 用 `GET /cases/{id}/events` 导出真实事件序列。
2. 脱敏后落到 `trajectory_cases.json`（保留 sequence 与 event_type 的因果顺序）。
3. `expected_violations_before` 填当时实际发生的违规 code；修复后的轨迹放进 `events_after`。
4. 跑 `scripts/run_landing_eval.py` 确认新 case 被拦截，即完成"线上事故 → 回归夹具"的闭环。
